...
a single space is used to separate the character codes and 3 spaces are used to 
separate words. 
...
please keep that in mind. This is mostly because otherwise the engine would 
simply ignore the tests, resulting in a "valid" solution.
Good luck!
*/
...
int main(){
    char* word = decode_morse(" Sabin badal "); //... .- -... .. -.   -... .- -.. .- .-..
    return 0;
}
...
char *decode_morse(const char* morse_code){
    ...
    for(int i = 0; i < size; i++){
        if(*(morse_code + i) == ' '){
            noOfSpaces++;
            if(*(morse_code + i) == ' ' && *(morse_code + (i-1)) == ' ' && *(morse_code + (i+1)) == ' '){
                noOfWordSpace += 3;
                wordSepSpace += 1;
            }
        }
    ...
    char* separatedWords[noOfSpaces + 1];
    for(int i = 0; i < (noOfSpaces + 1); i++){
        separatedWords[i] = (char*) malloc(10);
    }
    ...
    while (true)
    {
        if((*morse_code == ' ' || *morse_code == '\0') && flag == 1){
            *(temp + (j+1)) = '\0';
            if(strlen(temp) != 0){
             strcpy(*(separatedWords + index), temp);
                index++;   
            }
            memset(temp, 0, 10);
            j = 0;
